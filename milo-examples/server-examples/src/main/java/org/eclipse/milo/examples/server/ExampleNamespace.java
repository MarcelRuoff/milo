...
public class ExampleNamespace implements Namespace {
    ...
    private void addMethodNode(UaFolderNode folderNode) {
        ...
        UaMethodNode methodNodePost = UaMethodNode.builder(server.getNodeMap())
                ...
                .setBrowseName(new QualifiedName(namespaceIndex, "Post"))
                .setDisplayName(new LocalizedText(null, "Post"))
                .setDescription(
                    LocalizedText.english("Post to Marmotta."))
                .build();
        ...
        try {
            ...
            methodNodePost.setInvocationHandler(invocationHandlerPost);

            server.getNodeMap().addNode(methodNodePost);

            folderNode.addReference(new Reference(
                folderNode.getNodeId(),
                Identifiers.HasComponent,
                methodNodePost.getNodeId().expanded(),
                methodNodePost.getNodeClass(),
                true
            ));

            methodNodePost.addReference(new Reference(
            	methodNodePost.getNodeId(),
                Identifiers.HasComponent,
